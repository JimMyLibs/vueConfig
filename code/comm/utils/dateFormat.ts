 ...
 */
export default function dateFormat(timestamp: string | number = Date.now(), format: string = 'yyyy-M-d') {
  return getFormatDate(getJsonDate(timestamp), format)
}

// 生成时间对象
export function getJsonDate(timestamp: string | number) {
...
// 生成格式化时间字符串
export function getFormatDate(jsonDate: any, format: string) {
...
* 226天13时29分
 */
export function getDiffDay(original: any, minuend = Date.now()) {

  // 获得时间差值
  let diffTime: number = new Date(getCorrectTime(original)).getTime() - new Date(getCorrectTime(minuend)).getTime()

  // 一天的毫秒数
  let daySeconds = 24 * 3600 * 1000

  // 相差天数
  let diffDay: number = (parseInt as any)(diffTime / 1000 / 3600 / 24)

  // 剩余相差小时
  let diffHours = (parseInt as any)((diffTime - diffDay * daySeconds) / 1000 / 3600)

  // 剩余相差分钟
  let diffMinutes = (parseInt as any)((diffTime - diffDay * daySeconds - diffHours * 3600 * 1000) / 1000 / 60)

  // 剩余相差秒
  let diffSeconds = (parseInt as any)((diffTime - diffDay * daySeconds - diffHours * 3600 * 1000 - diffMinutes * 60 * 1000) / 1000)
  ...
  function concatTime(num: any, type: any) {
 ...
 */
export function getMoveDay(AddDayCount: number, isTwoDigit = true) {
  let dd = new Date()
...
// 数字如果为一位数，前面加0
function getTwoDigit(num: number | string): string {
...
// 对于传入时间数据的转换
function getCorrectTime(timestamp: number | string) {
  
  if ((isNaN as any)(timestamp)) {
