...
import { FetchOptions } from "comm/utils/PublicInterface";
// 引入json数据转换queryString方法
import { jsonToParams } from "comm/utils/url";
...
export default class Http {
   ...
   */
  toGet(url: string, options?: FetchOptions) {
   ...
   */
  toPost(url: string, options?: FetchOptions) {
   ...
   */
  toUpload(url: string, options: FetchOptions, isSetContentType = true) {
    let { method = "post", body, headers = {} } = this.getReqConf(options);

    let fetConf: Options = { method: method.toUpperCase() };

    let contentIndex = Object.keys(headers).findIndex(
      item => item.toLocaleLowerCase() === "content-type"
    );
  ...
  getGetConf(url: string, options: FetchOptions) {
    ...
    if (typeof body === "object" && body !== null) {
      body = "t" in body ? body : { ...body, t: Date.now() };
   ...
   * @param {undefined | object | string} options 详情见上述toGet中 "options 特别说明"
   */
  getPostConf(options: Options) {
    let { method, body, headers } = this.getReqConf(options);
    method = method ? method : "post";
    let conf = this.getContentFormat(headers, body);
    return { method: method.toUpperCase(), ...conf };
  }
   ...
   */
  getContentFormat(headers: any, body: any) {
    let formatBody: Options = {};
    if (typeof headers === "object") {
      let contentItem = Object.keys(headers).find(
        item => item.toLocaleLowerCase() === "content-type"
      );
   ...
   * @param {undefined | object | string} options 详情见上述toGet中 "options 特别说明"
   */
  getReqConf(options: FetchOptions): FetchOptions {
    let reqConf: Options = { body: "" };
   ...
   */
  toFetch(url: string, reqConf: any) {
    if (typeof url !== "string" || url.trim() === "") {
      throw new Error("fetch url is required and must be a string");
    }
