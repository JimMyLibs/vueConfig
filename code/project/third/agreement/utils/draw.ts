...
export default class Draw {
  ...
  degree: any
  constructor(canvas: any, degree: any, config = {}) {
    ...
    if (devicePixelRatio) {
      ...
      canvas.height = (height as any) * devicePixelRatio;
      canvas.width = (width as any) * devicePixelRatio;
      context.scale(devicePixelRatio, devicePixelRatio);
      context.fillStyle = "#ffffff";
      context.fillRect(0, 0, (width as any) * devicePixelRatio, (height as any) * devicePixelRatio);
    ...
    const optimizedMove = requestAnimationFrame ? (e: any) => {
  ...
  scale(width: number, height: number, canvas = this.canvas) {
  ...
  rotate(degree: any, image = this.canvas) {
    degree = ~~degree;
    if (degree !== 0) {
      const maxDegree = 180;
      const minDegree = -90;
